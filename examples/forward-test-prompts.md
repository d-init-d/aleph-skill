...
## D Research unavailable

Use $aleph-skill with D Research intentionally unavailable to simulate a public-policy intervention. Use only the host's lawful research tools, preserve explicit source provenance, keep public-role research separate from sealed roleplay, and checkpoint the run for a later execution if it cannot reach evidence saturation.

Expected behavior:

- Build the standard evidence map without fabricating a D Research ledger, HMAC sidecar, or import receipt.
