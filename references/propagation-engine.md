...
For cold-start simulation runs, the numerical execution trace (`execution-trace.json`) is dynamically generated by the propagation engine (`aleph.engine.numerical`), capturing state transitions, drawn parameters, and cryptographic hash chains directly from mathematical simulation without requiring pre-authored trace data. Agents specify the model structure (`nodes.json`, `edges.json`) and run parameters in `simulation-manifest.json`; calling `run_simulation.py` automatically executes propagation and emits an authentic, schema-compliant execution trace (`generation_mode: engine_derived`).

When converting a freshly initialized qualitative workspace to numerical execution, explicitly set `simulation_mode` to `deterministic` or `monte_carlo` and declare `artifact_paths.execution_trace` as `execution-trace.json`. That file must initially be absent or empty. The qualitative `propagation-trace.jsonl` template can remain as draft material; the declared execution trace takes precedence. A cold start targeting a legacy `.jsonl` path fails with `TRACE_FORMAT` before committing artifacts. An existing malformed trace is rejected and preserved; starting a new run must not silently replace invalid evidence.

For legacy formula 2.0 authored traces (`propagation-trace.jsonl`), deterministic replay via `replay_simulation.py` remains distinctly supported under `generation_mode: analyst_authored_legacy`. In that mode, every row must strictly bind to admitted edges and the declared run contract; an absent, empty, or mismatched trace fails closed.
