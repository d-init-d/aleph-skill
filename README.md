...
Aleph pins D Research 3.4.1 (scope-bound investigative routing, policy-aware 37-column ledger import, strict lead/evidence separation, machine-checkable interop contract, config-driven API pagination, explicit-intent HTTP methods, truthful read-only-by-default capability discovery, and hermetic social snapshot self-tests). Investigative operations remain workspace-confined and pass through the same locked gateway.
